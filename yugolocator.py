...
def add_boardpin_emoji(text_argument="./asciiartyugo.mapready.txt", coordinates: tuple[int,int] = None, ip_address = None, emoji = '📌', is_file: bool = True):
    ...
    def is_within_bounds(coord):
        lat, lon = coord
        return (
            top_left[0] >= lat >= bottom_right[0]
            and top_left[1] <= lon <= bottom_right[1]
        )
    ...
    def convert_to_map_coords(coord):
        lat, lon = coord
        x = int(
            (lon - top_left[1]) / (bottom_right[1] - top_left[1]) * width
        )  # (width +13))
        y = int(
            (top_left[0] - lat) / (top_left[0] - bottom_right[0]) * height
        )  # (height -1))
        return x, y
...
if __name__ == "__main__":
    ...
    coordinates = (
        44.809136541261275,
        20.45569482562331,
    )  # Example coordinates (importable directly from google maps using right click -> coordinates)
